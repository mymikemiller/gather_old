...
actor Gather {
  ...
  // Add an RSVP
  public shared (msg) func rsvp(rsvp : Rsvp, gatheringId : Nat) : async RsvpResult {
    // Get caller principal
    let callerId = msg.caller;
    ...
    switch (getUser(callerId)) {
      ...
      case (?user) {
        // We found the user submitting the rsvp
        switch (await getGathering(gatheringId)) {
          case null {
            return #err(#GatheringNotFound);
          };
          case (?gathering) {
            // we found a gathering to rsvp to
